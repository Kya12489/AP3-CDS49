...
class EleveModel extends SQL
{
    ...
    public function creer_eleve(string $nom, string $prenom, string $email, string $motDePasse, string $dateNaissance, string $numeroTelephone): bool
    {
        ...
        // Hachage du mot de passe avec PASSWORD_BCRYPT + PEPPER avant de le stocker dans la base de données.
        $pepper = getenv('PASSWORD_PEPPER') ?: '';
        $hashedPassword = password_hash($motDePasse . $pepper, PASSWORD_BCRYPT);
    ...
    public function connexion(string $email, string $motDePasse, string $token = ""): array
    {
        ...
        if ($eleve) {
            ...
            $pepper = getenv('PASSWORD_PEPPER') ?: '';
            if (!password_verify($motDePasse . $pepper, $eleve['motpasseeleve'])) {
    ...
    public function update(string $ideleve, string $nom, string $prenom, string $email, string $datenaissanceeleve, ?string $numeroTelephone = null, ?string $motDePasse = null): bool
    {
        ...
        if ($motDePasse !== null) {
            ...
            // HACHAGE DU MOT DE PASSE AVANT SAUVEGARDE DANS LA BASE DE DONNEES AVEC PASSWORD_DEFAULT + PEPPER
            $pepper = getenv('PASSWORD_PEPPER') ?: '';
            $params[':motDePasse'] = password_hash($motDePasse . $pepper, PASSWORD_BCRYPT);
        }
