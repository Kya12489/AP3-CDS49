...
class EleveModel extends SQL
{
     ...
     * @param string $prenom
     */
    public function creer_eleve(string $nom, string $prenom, string $email, string $motDePasse, string $dateNaissance, string $numeroTelephone): bool
    {
        ...
        // Hachage du mot de passe avec PASSWORD_BCRYPT avant de le stocker dans la base de données.
        $hashedPassword = password_hash($motDePasse, PASSWORD_BCRYPT);
        $motDePasse = $hashedPassword;
        
        // Préparer la requête d'insertion

        $query = "INSERT INTO eleve (nomeleve, prenomeleve, emaileleve, motpasseeleve, datenaissanceeleve, numeroteleleve) 
                  VALUES (:nom, :prenom, :email, :motDePasse, :dateNaissance, :numeroteleleve)";
        ...
        $params = [
            ...
            ':numeroteleleve' => $numeroTelephone // Numéro de téléphone optionnel, peut être mis à jour plus tard
        ];
    ...
    public function connexion(string $email, string $motDePasse, string $token = ""): array
    {
        ...
        if ($eleve) {
            ...
            if (!password_verify($motDePasse, $eleve['motpasseeleve'])) {
     ...
     * @return bool
     */
    public function update(string $ideleve, string $nom, string $prenom, string $email, string $datenaissanceeleve, ?string $numeroTelephone = null, ?string $motDePasse = null): bool
    {
        $pdo = $this->getPdo();
        ...
        // AJOUT DU CHAMP TELEPHONE DANS LA MISE A JOUR DES INFORMATIONS DE L'ELEVE -- LOT 1 --
        if ($numeroTelephone !== null) {
            $query .= ", numeroteleleve = :numeroTelephone";
            $params[':numeroTelephone'] = $numeroTelephone; // Ajouter le numéro de téléphone aux paramètres uniquement s'il est fourni - LOT 1 --            
        }
        
        // AJOUTER LE MOT DE PASSE S'IL EST FOURNI !! -- LOT 1 --

        if ($motDePasse !== null) {
            // Modification de la requête pour inclure la mise à jour du mot de passe
            $query .= ", motpasseeleve = :motDePasse";

            // HACHAGE DU MOT DE PASSE AVANT SAUVEGARDE DANS LA BASE DE DONNEES AVEC PASSWORD_DEFAULT
            $params[':motDePasse'] = password_hash($motDePasse, PASSWORD_BCRYPT);
        }
        ...
        if ($result) {
            ...
            SessionHelpers::login([
                ...
                'emaileleve' => $email,
                'datenaissanceeleve' => $datenaissanceeleve
                // Ne pas inclure le numéro de téléphone et le mot de passe dans la session pour des raisons de sécurité -- LOT 1 --
            ]);

            if ($numeroTelephone !== null) {
                // Mettre à jour le numéro de téléphone dans la session si fourni - LOT 1
                $connected = SessionHelpers::getConnected();
                $connected['numeroteleleve'] = $numeroTelephone;
                SessionHelpers::login($connected);
            }
    ...
    // Méthode de mise à jour du mot de passe uniquement
    public function updatePassword(string $ideleve, string $motDePasse): bool
    {
        $pdo = $this->getPdo();
    ...
    /**
     * Méthode pour récupérer un élève par son email.
     */
    ...
    /**
     * Méthode pour vérifier si un élève existe avec l'email donné.
     */
    public function existsByEmail(string $email): bool
    {
        $query = "SELECT COUNT(*) FROM eleve WHERE emaileleve = :email";
        $stmt = $this->getPdo()->prepare($query);
        $stmt->execute([':email' => $email]);
    ...
    /**
     * Méthode pour supprimer un élève par son email.
     */

    public function deleteByEmail(string $email): bool
    {
        $query = "DELETE FROM eleve WHERE emaileleve = :email";
        $stmt = $this->getPdo()->prepare($query);
